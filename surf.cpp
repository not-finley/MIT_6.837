...
Surface makeSurfRev(const Curve &profile, unsigned steps)
{
    ...
    for (unsigned i = 0; i < profile.size(); i++) {
        ...
        for (unsigned j = 0; j <= steps; j++) {
            ...
            Vector4f surfCalc = Vector4f(profile[i].V[0],profile[i].V[1],profile[i].V[2], 1.0f);
            Vector4f surfInit = rotatM * surfCalc;
            Vector3f surfVec = Vector3f(surfInit[0], surfInit[1], surfInit[2]);
            
            Vector3f surfNInit = rotatMSub*profile[i].N;
            
            surface.VV.push_back(surfVec);
            surface.VN.push_back(-1 * surfNInit);
        }
    }
    ...
    for (unsigned k = 0; k<surface.VV.size() - (steps + 1); k++) {
        ...
        if ((k+1)%(steps+1) != 0) {
            ...
            secTri = Tup3u(k+1, k+1+steps, k+2+steps);
        }
        
        surface.VF.push_back(firstTri);
        surface.VF.push_back(secTri);
    }
...
Surface makeGenCyl(const Curve &profile, const Curve &sweep )
{
    ...
    for (unsigned k = 0; k<surface.VV.size() - sweep.size(); k++) {
        ...
        if ((k+1)%(sweep.size()) != 0) {
            ...
            secTri = Tup3u(k+1, k+sweep.size(), k+1+sweep.size());
        }
